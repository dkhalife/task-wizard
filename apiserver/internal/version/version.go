...
var (
	...
	// Set via: go build -ldflags "-X taskwiz.app/core/internal/version.Version=1.0.0"
	Version = "dev"

	// BuildNumber is the CI build number injected at build time
	// Set via: go build -ldflags "-X taskwiz.app/core/internal/version.BuildNumber=123"
	BuildNumber = "0"

	// CommitHash is the git commit SHA injected at build time
	// Set via: go build -ldflags "-X taskwiz.app/core/internal/version.CommitHash=abc123"
	CommitHash = "unknown"
)
